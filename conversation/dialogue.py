...
def observe(conversation_history, observation):
    conversation_history.append(observation)

    return conversation_history


def reset_history(conversation_history):
    conversation_history = []

    return conversation_history


def set_input(conversation_history, situation_narrative="", role_instruction=""):
...
def generate(
    ...
    conversation_history = observe(conversation_history, user_response)

    input_text = set_input(conversation_history, situation_narrative, role_instruction)
    ...
    outputs = model.generate(
        input_ids, max_new_tokens=128, temperature=1.0, top_p=0.95, do_sample=True
    )
    # print("=====> OUTPUT : ", outputs)
    output = outputs[0]
    output = output.view(input_ids.size(0), -1)
    if output_summaries_only:
        # print("=====> INPUT SHAPE : ", input_ids.shape)
        # print("=====> OUTPUT SHAPE : ", output.shape)
        output = output[:, input_ids.shape[-1] :].cpu()
        output = output.view(-1)
    response = tokenizer.decode(
        output, skip_special_tokens=True, clean_up_tokenization_spaces=False
    )

    conversation_history = observe(conversation_history, response)
...
def conversation(
    ...
    **gen_kwargs,
    ...
    """Run generation"""
    fout = Path(out_file + ".chat").open("w", encoding="utf-8")
    fout_org = Path(out_file + ".dialogue").open("w", encoding="utf-8")
    ...
    for ind, sample in tqdm(enumerate(dataset.examples)):
        conversation_history = reset_history(conversation_history)
        situation_narrative = sample["narrative"]
        role_instruction = f"Generate a dialogue between {sample['speakers'][0]} and {sample['speakers'][1]} given above narrative."
        for j, user_response in enumerate(sample["dialogue"]):
            if j == 0:
                conversation_history = observe(conversation_history, user_response)

                input_text = set_input(
                    conversation_history, situation_narrative, role_instruction
                )
                ...
                chat = response.replace("<sep>", "")
                chat = chat.replace("<turn>", "")
                chat = chat.replace("<end>", "")
                chat = "".join(chat.splitlines()) + "\n"
                fout.write(chat)
                fout.flush()
            else:
                dialogue = "".join(user_response.splitlines())
                fout_org.write(dialogue)
                fout_org.flush()
        fout_org.write("\n")
    fout.close()
    fout_org.close()
    ...
    preds_decoded_text = [x for x in open(out_file + ".chat").readlines()]
    target_required = [x for x in open(out_file + ".dialogue").readlines()]
...
def get_args():
    ...
    parser.add_argument("--dataset_path", type=str, help="like soda_eval.json")
