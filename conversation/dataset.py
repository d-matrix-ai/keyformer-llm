...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.examples = [example for example in self.list_data_dict]

        # Getting random samples for evaluation
        if total_count_override > 0:
            self.rand_samples = np.random.randint(
                len(self.examples), size=(total_count_override)
            ).tolist()
            self.examples = [self.examples[i] for i in self.rand_samples]
