...
def scaled_multihead_dot_product_attention(
    ...
    if is_causal:
        # Recent + Attention Sink
        if not (q.size(2) == 1):
            req_tokens = int((attn_weight.shape[-1] * kv_cache) / 100)
            prompt_tokens = 4  # Attention Sinks
            recent_tokens = req_tokens - prompt_tokens
            req_tokens = (recent_tokens, prompt_tokens)

        token_mask = attn_weight.new_ones(b, h, 1, d, dtype=torch.float16)
        token_mask = token_mask.to(torch.bool)

        # activate most recent k-cache
        token_mask[:, :, :, : -req_tokens[0]] = False
        # activate attention sinks
        token_mask[:, :, :, : req_tokens[1]] = True

        sparse_len = req_tokens[0] + req_tokens[1]

        # ==> Reducing KV cache
        v_new = v[token_mask.squeeze()]
        v_new = rearrange(v_new, "(b h s) d -> b h s d", h=kv_n_heads, s=sparse_len)

        k_new = k.transpose(2, 3)[token_mask.squeeze()]
